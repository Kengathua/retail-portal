...
echo /dev/tcp/$PGHOST/5432 >/dev/null 2>&1
...
echo $password | sudo -u postgres -c '\l' | grep "root" > /dev/null 2>&1
if [ $? == 0 ]
then
  echo "Root db exist"
else
...
echo $password | sudo -u postgres -c '\du' | grep "root" > /dev/null 2>&1
if [ $? == 0 ]
then
...
echo $password | sudo -u postgres -c '\l' | grep "demo" > /dev/null 2>&1
...
echo $password | sudo -u postgres -c '\du' | grep "demo" > /dev/null 2>&1
if [ $? == 0 ]
then
  echo "User exist"
else
  ...
  echo $password | sudo -u postgres -c 'grant all privileges on database "demo" to demo;'
  echo "User created"
fi
