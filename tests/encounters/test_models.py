...
class TestEncounter(TestEnterPriseSetupRules):
    ...
    def test_create_encounter(self, mock_process_customer_encounter):
        ...
        baker.make(
            InventoryInventoryItem, inventory=self.master_inventory, inventory_item=inventory_item1)
        baker.make(
            InventoryInventoryItem, inventory=self.master_inventory, inventory_item=inventory_item2)
        baker.make(
        ...
        baker.make(CatalogCatalogItem, catalog=self.default_catalog, catalog_item=catalog_item1)
        baker.make(CatalogCatalogItem, catalog=self.default_catalog, catalog_item=catalog_item2)
        customer = baker.make(
