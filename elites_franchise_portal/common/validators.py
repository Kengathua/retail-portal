...
def franchise_elites_code_validator(code):
    """Validate franchise code."""
    franchise_code_pattern = "[A-Z]+-+[A-Z]+/+[A-Z]+-+[A-Z]+/+[0-9]+-+[0-9]"

    if not (re.search(franchise_code_pattern, code)):
...
def items_elites_code_validator(code):
    """Validate Item code."""
...
def units_elites_code_validator(code):
    """Validate Units code."""
